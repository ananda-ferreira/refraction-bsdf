...
vec3 GetAlbedo(SurfaceData data)
{
	...
	return data.albedo; // instead of albedo, rgba?
}
...
// for transmission take eta into account?
...
// diffuse and specular are calculated by ComputeDiffuseIndirectLighting and ComputeSpecularIndirectLighting
...
vec3 ComputeSpecularLighting(SurfaceData data, vec3 lightDir, vec3 viewDir)
{
	...
	float cosThetaI = ClampedDot(data.normal, lightDir);
	float cosThetaO = ClampedDot(data.normal, viewDir);

	return vec3((D * G) / (4.0f * cosThetaO * cosThetaI + 0.00001f));
}
